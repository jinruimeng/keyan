...
def KMeansOushi_costFunction(dataSet, k, npower, newDimension=5):
    ...
    for i in range(m):
        ...
        maxIs.append(maxI)
        covs.append(np.cov(dataSet[i], rowvar=False))

    # 第1步 初始化centroids
    centroidList = randCent2(covs, k)
    UList = []
    for i in range(k):
